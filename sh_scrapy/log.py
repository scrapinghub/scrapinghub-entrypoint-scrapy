...
def initialize_logging():
    ...
    with suppress(ImportError):
        import boto  # noqa: F401  # boto overrides its logger at import time
    ...
    def _get_log_item(self, ev):
        ...
        if fmt:
            try:
                ...
                msg = f"UNABLE TO FORMAT LOG MESSAGE: fmt={fmt!r} ev={ev!r}"
                level = logging.ERROR
...
class StdoutLogger(txlog.StdioOnnaStick):
    ...
    def writelines(self, lines):
        for line in lines:
            self._logprefixed(to_unicode(line, self.encoding))
