...
No installed distribution declares a `scrapy.settings` entry point.

Declare one in your project, pointing at its settings module:

    # pyproject.toml
    [project.entry-points.scrapy]
    settings = "your_project.settings"
...
def _fatalerror():
    ...
    if _sentry_dsn:
        try:
            ...
            print(
                "HWORKER_SENTRY_DSN is set but python-raven is not installed",
                file=_sys_stderr,
            )
        else:
            try:
                ...
                print(
                    ...
                    "Error when sending fatal error to sentry:",
                    err,
                    file=_sys_stderr,
                )
...
def _run_pkgscript(argv):
    ...
    _run_script(d, scriptname, {"__name__": "__main__"})


def _get_script_code_and_path(
    dist: PathDistribution, script_name: str
...
def _run_usercode(
    spider, args, apisettings_func, log_handler=None, commands_module=None
):
    try:
        ...
        logger.exception("Settings initialization failed")
        raise

    try:
        _run(args, settings)
    except Exception:
        logger.exception("Job runtime exception")
        raise
...
def _launch():
    try:
        ...
        from sh_scrapy.env import decode_uri, get_args_and_env
        ...
        from sh_scrapy.settings import populate_settings  # noqa: F401
...
def shub_image_info():
    ...
    _run_usercode(
        None,
        ["scrapy", "shub_image_info", *sys.argv[1:]],
        _get_apisettings,
        commands_module="sh_scrapy.commands",
    )
