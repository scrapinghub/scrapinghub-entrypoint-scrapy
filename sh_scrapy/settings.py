...
REPLACE_ADDONS_PATHS = {
    "hworker.bot.ext.persistence.DotScrapyPersistence": "scrapy_dotpersistence.DotScrapyPersistence",
    "scrapylib.deltafetch.DeltaFetch": "scrapy_deltafetch.DeltaFetch",
    "scrapylib.magicfields.MagicFieldsMiddleware": "scrapy_magicfields.MagicFieldsMiddleware",
    "scrapylib.querycleaner.QueryCleanerMiddleware": "scrapy_querycleaner.QueryCleanerMiddleware",
    "scrapylib.splitvariants.SplitVariantsMiddleware": "scrapy_splitvariants.SplitVariantsMiddleware",
    "scrapy.contrib.throttle.AutoThrottle": "scrapy.extensions.throttle.AutoThrottle",
}
...
class EntrypointSettings(Settings):
    """Settings object that starts out empty, so that the settings set on it
    can be told apart from Scrapy defaults when merging."""
...
def _update_old_classpaths(settings):
    ...
    """
    for setting_key in settings.attributes:
        setting_value = settings[setting_key]
        # Dict-like settings are BaseSettings when they come from Scrapy
        # defaults, and plain dicts when they come from a settings module.
...
def _load_addons(addons, settings, merged_settings, priority=0):
    ...
    for addon in addons:
        ...
        try:
            ...
            message = f"Addon import error {addon_path}:\n {exc}"
            if on_missing_addons == "warn":
                logger.warning(message)
                continue
            if on_missing_addons == "error":
...
def _populate_settings_base(apisettings, defaults_func, spider=None):
    ...
    if spider:
        ...
        merged_settings.set("JOBDIR", tempfile.mkdtemp(prefix="jobdata-"), priority=40)
    ...
    settings.setdict(
        {
            ...
            "LOG_LEVEL": "INFO",
            "LOG_ENABLED": False,
            "TELNETCONSOLE_HOST": "0.0.0.0",  # noqa: S104  # to reach telnet from host
        },
        priority="cmdline",
    )
