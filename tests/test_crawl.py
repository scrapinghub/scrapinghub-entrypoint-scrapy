...
from tests.utils import call_command, create_project

try:
    import scrapy_spider_metadata  # noqa: F401
except ImportError:
...
@mock.patch.dict(
    os.environ, {"HWORKER_SENTRY_DSN": "hw-sentry-dsn", "SENTRY_DSN": "sentry-dsn"}
)
...
def test_fatal_error_ignore_IOError(trace_print):
    trace_print.side_effect = OSError("some error")
    try:
...
def test_fatal_error_sentry_with_mock_ignore_errors(sentry_dsn):
    ...
    raven_stub.Client = mock.Mock()
    sentry_client = raven_stub.Client.return_value
    sentry_client.captureException.side_effect = OSError("error")
    try:
        sys.modules["raven"] = raven_stub
        raise ValueError("some exception")
    except Exception:
...
@mock.patch.dict(os.environ, {"SHUB_SETTINGS": "data:;base64,ImhlbGxvIHdvcmxkIg=="})
...
def test_run_pkg_script_distribution_not_found(entry_points_mock):
...
def test_run_scrapy(execute_mock):
    ...
    assert execute_mock.call_args == ({"settings": {"SETTING": "VALUE"}},)
...
@mock.patch.dict(os.environ, {"SHUB_SETTINGS": '{"project_settings": {"SETTING....'})
...
@mock.patch.dict(
    os.environ, {"SHUB_SETTINGS": '{"project_settings": {"SETTING_TEST": "VAL"}}'}
)
@mock.patch("sh_scrapy.crawl._run")
...
@mock.patch.dict(
    os.environ, {"SHUB_SETTINGS": '{"project_settings": {"LOG_LEVEL": 10}}'}
)
@mock.patch("sh_scrapy.crawl._run")
def test_run_usercode_with_loghandler(mocked_run):
    loghandler = mock.Mock()
    _run_usercode(
        "py:script.py", ["py:script.py", "arg1"], _get_apisettings, loghandler
    )
    assert mocked_run.called
...
def test_image_info(tmp_path):
    ...
    out, _err = call_command(project_dir, "shub-image-info")
...
@pytest.mark.skipif(
    not SPIDER_METADATA_AVAILABLE, reason="scrapy-spider-metadata is not installed"
)
