...
from sh_scrapy.settings import (
    EntrypointSettings,
    _enforce_required_settings,
    _get_action_on_missing_addons,
    _get_component_base,
    _load_addons,
    _load_default_settings,
    _maybe_load_autoscraping_project,
    _populate_settings_base,
    _update_old_classpaths,
    populate_settings,
)
...
def test_update_settings_per_key_priorities_new_behaviour():
    ...
    assert dict(test["ITEM_PIPELINES"]) == {"test.path1": 100, "test.path2": 200}
...
def test_maybe_load_autoscraping_project_ok():
    ...
    _maybe_load_autoscraping_project(result)
    assert result["ITEM_PIPELINES"] == {"slybot.dupefilter.DupeFilterPipeline": 0}
...
@mock.patch.dict(
    "sh_scrapy.settings.REPLACE_ADDONS_PATHS",
...
def test_populate_settings_base_dont_fail():
...
def test_populate_settings_override_settings():
    msg = {}
    for section in ["project_settings", "organization_settings", "job_settings"]:
...
def test_populate_settings_with_spider():
    msg = {
        "project_settings": {"JOBDIR": "by_project"},
...
def test_populate_settings_keep_user_priorities(get_settings_mock):
    ...
    assert (
        result.getdict("SPIDER_MIDDLEWARES_BASE")["scrapy.utils.misc.load_object"] == 1
    )
    assert (
        result.getdict("EXTENSIONS_BASE")["sh_scrapy.extension.HubstorageExtension"]
        is None
    )
    autothrottles = [
...
def test_populate_settings_unique_update_dict():
    monitoring_dict = {"SPIDER_OPENED": {"failed_actions": []}}
...
def test_populate_settings_keep_user_priorities_oldpath(get_settings_mock):
    ...
    assert result.getdict("EXTENSIONS_BASE")[autothrottles[0]] == 0


def test_load_default_settings():
    result = Settings(
        {
            "EXTENSIONS_BASE": {"sh_scrapy.extension.HubstorageExtension": 50},
            "SPIDER_MIDDLEWARES_BASE": {},
        }
    )
    ...
    assert (
        "sh_scrapy.middlewares.HubstorageDownloaderMiddleware" in downloader_middlewares
    )
...
def test_load_default_settings_mem_limit():
    result = Settings({"EXTENSIONS_BASE": {}, "SPIDER_MIDDLEWARES_BASE": {}})
    _load_default_settings(result)
...
def test_update_old_classpaths_not_string():
    ...
    test_value = {"scrapy.exporter.CustomExporter": 1, 123: 2, CustomObject: 3}
